...
export default class Core {
      ...
      switch (this.options.position) {
        case 'bottom-center':
          this._container.style.left =
            `${window.outerWidth / 2 - containerRect.width / 2}px`;
        ...
        case 'mid-center':
          ...
          this._container.style.bottom =
            `${window.outerHeight / 2 - containerRect.height / 2}px`;
          break;
  ...
  addToDom() {
    ...
    if (!_container) {
      ...
      for (const ele of _container.children) {
    ...
    if (
      ...
      const previousToastCount =
                _container.querySelectorAll('.toaste-single').length;
      const extraToastCount = previousToastCount - this.options.stack;
      ...
      if (extraToastCount > 0) {
        ...
        for (const toast of toastsRemoving) {
          _container.removeChild(toast);
        }
      }
    }
  ...
  processLoader() {
    ...
    const loader = this._toastEl.querySelector('.toaste-loader');
  ...
  animate() {
    ...
    if (this.canAutoHide()) {
      this.autoCloseTimeout = window.setTimeout(
          this.closeToast.bind(this, that),
  ...
  reset(all = true) {
    if (all) {
      document.querySelectorAll('.toaste-wrap').forEach((el) => {
        el.remove();
      });
    } else {
  ...
  closeToast(toastEInstance, event) {
    ...
    if (toastEInstance.options.showHideTransition === 'fade') {
      FadeAnimation.fadeElement(
          toastEInstance._toastEl,
          animationOptions).play();
